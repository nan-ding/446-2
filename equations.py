...
class SHEquation:
    ...
    def __init__(self, domain, u):
        ...
        if dtype == np.complex128:
            diag = 1.3*np.ones(x_basis.N)+2*(1j**2)*x_basis.wavenumbers(dtype)**2 + (1j**4)*x_basis.wavenumbers(dtype)**4
            p.L = sparse.diags(diag)
        elif dtype == np.float64:
            diag = 1.3*np.ones(x_basis.N)-2*x_basis.wavenumbers(dtype)**2 + x_basis.wavenumbers(dtype)**4
            p.L = sparse.diags(diag)
