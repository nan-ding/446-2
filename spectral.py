...
class Fourier(Basis):
    ...
    def _transform_to_grid_complex(self, data, axis, scale):
        N = self.N 
        coeffs = np.zeros(int(scale*N), dtype=np.complex128)
    ...
    def _transform_to_coeff_complex(self, data, axis):
        ...
        fft = scipy.fft.fft(data, axis=axis)/len(data) #normalization
    ...
    def _transform_to_grid_real(self, data, axis, scale):
        N=self.N
        coeffs = np.zeros(int(scale*N//2)+1, dtype=np.complex128)
        #fill the first N/2 terms with a_n and b_n
        coeffs.real[0:len(data)//2] = data[0::2]
        coeffs.imag[0:len(data)//2] = data[1::2]
    ...
    def _transform_to_coeff_real(self, data, axis):
        ...
        rfft = scipy.fft.rfft(data, axis=axis)/(len(data)/2) #normalization by testing 
